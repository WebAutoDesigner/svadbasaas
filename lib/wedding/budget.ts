...
export async function listBudget(agencyId: string, weddingId: string) {
  return tenantScope(agencyId, async () => {
    ...
    return getDb().budgetItem.findMany({
      where: { weddingId },
...
export async function budgetSummary(
  ...
  return tenantScope(agencyId, async () => {
    const wedding = await getDb().wedding.findFirst({
    ...
    const agg = await getDb().budgetItem.aggregate({
      where: { weddingId },
...
export async function ensureDefaultCategories(
  ...
  return tenantScope(agencyId, async () => {
    ...
    const count = await getDb().budgetItem.count({ where: { weddingId } });
...
export async function addBudgetItem(
  ...
  return tenantScope(agencyId, async () => {
    ...
    const item = await getDb().budgetItem.create({
      data: {
        weddingId,
        name: input.name,
...
export async function updateBudgetItem(
  ...
  return tenantScope(agencyId, async () => {
    if (!(await assertWedding(agencyId, weddingId))) return err("NOT_FOUND");
    const updated = await getDb().budgetItem.updateMany({
...
export async function deleteBudgetItem(
  ...
  return tenantScope(agencyId, async () => {
    if (!(await assertWedding(agencyId, weddingId))) return err("NOT_FOUND");
    const deleted = await getDb().budgetItem.deleteMany({
      where: { id: itemId, weddingId },
    });
