...
export function GuestsBoard({
  ...
  guests: Guest[];
  summary: Summary;
  unseen: { count: number; items: { summary: string; createdAt: string | Date }[] };
}) {
  ...
  const groups = useMemo(
    () =>
      Array.from(
        new Set(guests.map((g) => g.groupLabel).filter((g): g is string => !!g)),
      ),
    [guests],
  );
  ...
  const filtered = guests.filter(
    (g) =>
      ...
      (groupFilter === "" || g.groupLabel === groupFilter),
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        ...
        <Stat label="Придут" value={summary.coming} />
        <Stat label="Не придут" value={summary.notComing} />
        <Stat label="Под вопросом" value={summary.maybe} />
        <Stat label="Не ответили" value={summary.noAnswer} />
      </div>
      ...
      {filtered.length === 0 ? (
        <p className="text-muted-foreground">
          {guests.length === 0
            ? "Гостей пока нет. Добавьте имена — статус проставите вручную."
            : "Нет гостей под выбранные фильтры."}
        </p>
      ) : (
...
function QuickAdd({ weddingId }: { weddingId: string }) {
  ...
  function submit() {
    ...
    if (!value) return;
    start(async () => {
      const res = await quickAddGuestAction(weddingId, { name: value });
  ...
  return (
    <div className="border rounded-md p-3 flex flex-col md:flex-row md:items-center gap-3">
      ...
      <select
        ...
        onChange={(e) =>
          start(async () => {
            const res = await setGuestStatusAction(weddingId, guest.id, {
      ...
      <div className="flex gap-2 shrink-0">
        ...
        <Button
          ...
          onClick={() =>
            start(async () => {
              const res = await deleteGuestAction(weddingId, guest.id);
              if (res.error) toast.error(res.error);
            })
          }
        >
...
function GuestForm({
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    start(async () => {
      const res =
        mode === "create"
          ? await addGuestAction(weddingId, input)
          : await updateGuestAction(weddingId, guest!.id, input);
      if (res.error) toast.error(res.error);
      else {
